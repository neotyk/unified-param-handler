...
describe('Utility Functions (src/utils.js)', () => {
    ...
    describe('Logging Functions', () => {
        test('logError always logs to console.error', () => {
            ...
            expect(console.error).toHaveBeenCalledWith('[Unified Param Handler Error]: Test Error Message');
        });
        ...
        test('isDebugMode reflects window.location.search', () => {
            ...
            // Set other params
            Object.defineProperty(window.location, 'search', { value: '?other=true&debug=true&more=stuff', writable: true });
            expect(utils.isDebugMode()).toBe(true);

            // No debug param
            Object.defineProperty(window.location, 'search', { value: '?other=true', writable: true });
            expect(utils.isDebugMode()).toBe(false);
        });
        ...
        test('logDebug logs only when isDebugMode() is true', () => {
            ...
            expect(console.log).not.toHaveBeenCalledWith(expect.stringContaining('Debug off message')); // Ensure it wasn't called

            // Case 2: Debug mode ON
            Object.defineProperty(window.location, 'search', { value: '?param=test&debug=true', writable: true });
            utils.logDebug('Debug on message', { data: 1 });
            expect(console.log).toHaveBeenCalledWith('[Unified Param Handler] Debug on message', { data: 1 }); // Ensure it was called
        });
        ...
        test('startGroup groups only when isDebugMode() is true', () => {
            ...
            expect(console.groupCollapsed).toHaveBeenCalledWith('[Unified Param Handler] Test Group On Collapsed');
            ...
            expect(console.group).toHaveBeenCalledWith('[Unified Param Handler] Test Group On Expanded');
        });
    ...
    describe('Cookie Functions', () => {
        test('parseCookies correctly parses various cookie strings', () => {
            ...
            expect(console.log).toHaveBeenCalledWith('[Unified Param Handler] Parsed cookies:', {
                test1: 'value1',
                test2: 'value2',
            });
            Object.defineProperty(window.location, 'search', { value: '', writable: true }); // Turn debug off
        });
        ...
        test('getCookie retrieves existing cookie', () => {
            ...
            expect(console.log).toHaveBeenCalledWith('[Unified Param Handler] Getting cookie \'my_cookie\':', 'retrieved_value');
        ...
        test('getCookie returns null for non-existent cookie', () => {
            ...
            expect(console.log).toHaveBeenCalledWith('[Unified Param Handler] Getting cookie \'not_here\':', undefined);
        ...
        test('setCookie creates correctly formatted cookie string', () => {
            ...
            const expectedDate = new Date(Date.now() + 10 * 24 * 60 * 60 * 1000).toUTCString();
            ...
            // Check log
            expect(console.log).toHaveBeenCalledWith('[Unified Param Handler] Cookie set: new_cookie = some&value (Expires in 10 days)');
        ...
        test('setCookie creates session cookie if days is 0 or undefined', () => {
            ...
            // Check log
            expect(console.log).toHaveBeenCalledWith('[Unified Param Handler] Cookie set: session_cookie = temp (Expires in undefined days)');
    ...
    describe('Formatting Functions', () => {
        test('getSubdomainIndex calculates index correctly', () => {
            Object.defineProperty(window.location, 'hostname', { value: 'www.example.com', writable: true });
            expect(utils.getSubdomainIndex()).toBe(2);

            Object.defineProperty(window.location, 'hostname', { value: 'example.co.uk', writable: true });
            ...
            Object.defineProperty(window.location, 'hostname', { value: '192.168.1.1', writable: true });
            expect(utils.getSubdomainIndex()).toBe(1); // Default for IP

            Object.defineProperty(window.location, 'hostname', { value: 'com', writable: true });
            expect(utils.getSubdomainIndex()).toBe(0); // Specific check
        });

        test('formatFbClickId produces correct FB format string', () => {
            Object.defineProperty(window.location, 'hostname', { value: 'app.example.com', writable: true }); // index = 2
    ...
    describe('URL Parameter Utilities', () => {
        test('URL_PARAMS correctly reflects window.location.search', () => {
            ...
            */
            ...
            expect(utils.URL_PARAMS.get('any')).toBeNull(); // Because setup resets search to ''
        });
    });
});